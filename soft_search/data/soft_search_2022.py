...
ALL_SOFT_SEARCH_2022_DATASET_FIELDS = [
    ...
    for a in dir(SoftSearch2022DatasetFields) if "__" not in a
]
...
def _prepare_soft_search_2022(raw: Union[str, Path, pd.DataFrame]) -> Path:
    ...
    # Remap values
    software_values_map = {
        "yes": SoftwareOutcomes.SoftwarePredicted, "no": SoftwareOutcomes.SoftwareNotPredicted,
    }
    df[SoftSearch2022DatasetFields.stated_software_was_created] = df[SoftSearch2022DatasetFields.stated_software_was_created].map(software_values_map)
    df[SoftSearch2022DatasetFields.stated_software_will_be_created] = df[SoftSearch2022DatasetFields.stated_software_will_be_created].map(software_values_map)
...
def load_joined_soft_search_2022() -> pd.DataFrame:
    ...
    software_pre = df[[SoftSearch2022DatasetFields.id_, SoftSearch2022DatasetFields.abstractText, SoftSearch2022DatasetFields.stated_software_will_be_created]]
    software_post = df[[SoftSearch2022DatasetFields.id_, SoftSearch2022DatasetFields.projectOutComesReport, SoftSearch2022DatasetFields.stated_software_was_created]]

    # Map column values
    software_pre = software_pre.rename(columns={
        SoftSearch2022DatasetFields.abstractText: "text",
        SoftSearch2022DatasetFields.stated_software_will_be_created: "label",
    })
    software_post = software_post.rename(columns={
