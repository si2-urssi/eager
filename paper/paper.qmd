...
We trained multiple predictive models using different model architectures and using the two different predictive sources available (the award's abstract text or project outcomes report text). As not all awards in our set had concluded their research, the models trained on the project outcomes report text used a reduced size dataset.

We trained three different models for each of the predictive sources and include a custom regular expression as a baseline for comparison. The models trained include a TF-IDF Word Embedding Logistic Regression model, a Semantic Embedding Logistic Regression model, and a Fine-tuned Transformer model.
...
#| code-fold: true
#| warning: false
#| label: tbl-models-from-abstract
#| tbl-cap: Model Results from Training using Abstract Text

model_results[model_results["predictive_source"] == "abstract-text"].reset_index(drop=True)
```

```{python}
#| code-fold: true
#| warning: false
#| label: tbl-models-from-project-outcomes
#| tbl-cap: Model Results from Training using Project Outcomes Reports Text

model_results[model_results["predictive_source"] == "project-outcomes"].reset_index(drop=True)
```
